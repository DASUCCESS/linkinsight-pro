            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-6">
                ...
                <form method="POST" action="{{ route('admin.cms.pages.update', $page) }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <input type="text" name="slug" value="{{ old('slug', $page->slug) }}"
                                   class="w-full rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                            <p class="mt-1 text-[11px] text-slate-400">Homepage still renders as / even if slug is "home".</p>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-slate-100 dark:border-slate-800 pt-4 mt-2">
                        <p class="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase mb-3">Content</p>
                        <textarea name="content" rows="6"
                                  class="w-full rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500">{{ old('content', $page->content) }}</textarea>
                    </div>

                    <div class="border-t border-slate-100 dark:border-slate-800 pt-4 mt-2">
                        <p class="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase mb-3">SEO Metadata</p>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="meta_description" rows="3"
                                      class="w-full rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500">{{ old('meta_description', $page->meta_description) }}</textarea>
                        </div>
                        ...
                        <div class="mb-2">
                            ...
                            <textarea name="json_ld" rows="3"
                                      class="w-full rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs font-mono text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500">{{ old('json_ld', $page->json_ld ? json_encode($page->json_ld, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) : '') }}</textarea>
                    ...
                    <div class="flex justify-end pt-2">
                        <button type="submit"
                                class="px-4 py-2 rounded-full text-sm font-semibold shadow-xl cursor-pointer bg-gradient-to-r from-indigo-500 to-sky-500 text-white transition transform duration-150 hover:scale-[var(--hover-scale)]">
                            Save Page
                        </button>
                    </div>
                </form>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-4 text-sm">
                ...
                @if($page->is_home)
                    <a href="{{ route('admin.cms.sections.index', $page) }}"
                       class="mt-3 inline-flex items-center px-3 py-1.5 rounded-full text-[11px] font-semibold bg-gradient-to-r from-indigo-500 to-sky-500 text-white shadow cursor-pointer transition transform hover:scale-[var(--hover-scale)]">
